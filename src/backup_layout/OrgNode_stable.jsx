...
/* ── Individual Card ── */
const OrgNodeCard = ({ node, person, selected, childCount, assetCount, onSelect, onAddChild, onEditNode, onShowPerson, canEdit, bgColor, borderColor, collapsed, onToggleCollapse }) => {
  const isApoio = node.subtipo === "apoio";
  const displayPhoto = person?.photo || node.foto;
  return (
    <div className="org-card-wrap">
      ...
      >
        <div className="org-card-top">
          <div className="av" style={{ cursor: displayPhoto ? "zoom-in" : "default" }} onClick={(e) => { if(displayPhoto) { e.stopPropagation(); window.dispatchEvent(new CustomEvent('open-photo', { detail: displayPhoto })); } }}>
...
const ApoioBranch = ({ node, getChildren, personMap, selectedId, onSelect, onAddChild, onEditNode, onShowPerson, canEdit, directAssetCount, parentHex }) => {
  ...
  return (
    <li className="apoio-card-box">
      <div style={{ position: "relative" }}>
        <OrgNodeCard
          node={node} person={personMap?.get(node.personId)} selected={selectedId === node.id} childCount={childCh.length}
      ...
      {!collapsed && childCh.length > 0 && (
        <ul className="tree tree-apoio" style={{ "--connector-color": connColor }}>
          {childCh.map((child) => (
            <OrgBranch
              key={child.id} node={child} getChildren={getChildren} personMap={personMap} selectedId={selectedId}
...
/* ── Branch (recursive) ── */
const OrgBranch = ({ node, getChildren, personMap, selectedId, onSelect, onAddChild, onEditNode, onExpand, onShowPerson, directAssetCount, canEdit, parentHex, isFocusRoot, onReturnFromFocus }) => {
  const [collapsed, setCollapsed] = useState(false);

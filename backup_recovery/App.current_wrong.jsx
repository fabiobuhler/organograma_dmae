    ...
    <div className="modal-overlay" style={{ zIndex: 5200 }}>
      <div className="modal-content narrow system-alert-modal">
        <div className="modal-header">
          ...
          <p style={{ fontSize: 11, color: "var(--n600)", marginTop: 4 }}>{dialog.subtitle || "Confirme antes de continuar."}</p>
        </div>
        <div className="modal-body">
          <div className={`system-alert-box ${isDanger ? "error" : "warning"}`}>
            <div className="system-alert-icon">
              {isDanger ? <Trash2 size={22} /> : <AlertTriangle size={22} />}
            </div>
            <div className="system-alert-text">
              <div className="system-alert-title">{dialog.title || "Confirmar ação"}</div>
              <div className="system-alert-message">
        ...
        <div className="modal-footer">
          <button type="button" className="btn btn-outline btn-xs" onClick={onCancel}>Cancelar</button>
          <button type="button" className={isDanger ? "btn btn-danger btn-xs" : "btn btn-primary btn-xs"} onClick={onConfirm} autoFocus>{dialog.confirmText || "Confirmar"}</button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const saveAssetType = useCallback(async () => {
    ...
    if (!category || !name) {
      showSystemAlert("Preencha categoria e nome.", { title: "Campos obrigatórios", type: "warning" });
      return;
    }

    const duplicate = assetTypes.find((item) => {
      const sameId = editAssetTypeId && (item.id === editAssetTypeId || item._localId === editAssetTypeId);
      return !sameId && item.category.toLowerCase() === category.toLowerCase() && item.name.toLowerCase() === name.toLowerCase();
    });

    if (duplicate) {
      showSystemAlert("Já existe um tipo cadastrado com este grupo e nome.", { title: "Tipo duplicado", type: "warning" });
      return;
    }
    ...
    const payload = {
      id: assetTypeForm.id || (editAssetTypeId && String(editAssetTypeId).length > 20 ? editAssetTypeId : undefined),
      category: category,
      name: name
    };

    try {
      let savedPayload = { ...payload, id: payload.id || currentId, _localId: localId };

      if (supabase) {
        if (editAssetTypeId && assetTypeForm.id) {
          const { data, error } = await supabase.from("asset_types").update({ category, name }).eq("id", assetTypeForm.id).select().single();
          if (error) throw error;
          savedPayload = data || savedPayload;
        } else {
          const { data, error } = await supabase.from("asset_types").insert({ category, name }).select().single();
          if (error) throw error;
          savedPayload = data || savedPayload;
        }
      }
      ...
      setAssetTypes((current) => {
        if (editAssetTypeId) {
          return current.map((item) => {
            const match = (item.id && item.id === editAssetTypeId) || item._localId === editAssetTypeId;
      ...
      resetAssetTypeForm();
      showSystemAlert(editAssetTypeId ? "Tipo atualizado!" : "Tipo adicionado!", { title: "Concluído", type: "success" });
    } catch (error) {
      console.error("Erro ao salvar tipo:", error);
      showSystemAlert(`Erro ao salvar: ${error.message}`, { title: "Erro", type: "error" });
    }
  }, [assetTypeForm, assetTypes, editAssetTypeId, resetAssetTypeForm, showSystemAlert]);
  ...
  const requestDeleteAssetType = useCallback((item) => {
    ...
    const usedAssets = assets.filter(a => a.category?.toLowerCase() === item.category?.toLowerCase() && a.type?.toLowerCase() === item.name?.toLowerCase());
    ...
    setConfirmDialog({
      ...
      type: "danger",
      confirmText: "Excluir",
      message: `Deseja realmente excluir o tipo abaixo?\n\nGrupo: ${item.category}\nTipo: ${item.name}${usedAssets.length > 0 ? `\n\nAtenção: existem ${usedAssets.length} ativos usando este tipo.` : ""}`,
      onConfirm: async () => {
        try {
          if (supabase && item.id) {
          ...
          setAssetTypes(current => current.filter(t => (t.id || t._localId || `${t.category}-${t.name}`) !== typeId));
          if (editAssetTypeId === typeId) resetAssetTypeForm();
          showSystemAlert("Tipo excluído!", { title: "Concluído", type: "success" });
        } catch (error) {
          console.error("Erro ao excluir:", error);
          showSystemAlert(`Erro ao excluir: ${error.message}`, { title: "Erro", type: "error" });
        }
      }
    });
  }, [assets, editAssetTypeId, resetAssetTypeForm, showSystemAlert]);
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <div style={{ background: "var(--n50)", padding: 12, borderRadius: 8, marginBottom: 16 }}>
                <h3 style={{ fontSize: 12, marginBottom: 8 }}>{editAssetTypeId ? "Editar Tipo" : "Novo Tipo"}</h3>
                <div className="fr" style={{ gap: 8, alignItems: "flex-end" }}>
                  <div className="fg" style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    <button className="btn btn-primary" style={{ height: 36 }} onClick={saveAssetType}>
                      {editAssetTypeId ? <Save size={14} /> : <Plus size={14} />}
                      {editAssetTypeId ? "Salvar" : "Adicionar"}
                    </button>
                    {editAssetTypeId && (
                      <button className="btn btn-outline" style={{ height: 36 }} onClick={resetAssetTypeForm}>
                        <X size={14} />
                      </button>
                    )}
                  </div>
              ...
              <div style={{ maxHeight: 300, overflowY: "auto" }}>
                <table style={{ width: "100%", fontSize: 12 }}>
                  ...
                  <tbody>
                    {assetTypes.sort((a,b) => a.category.localeCompare(b.category) || a.name.localeCompare(b.name)).map((t) => {
                      const tId = t.id || t._localId || `${t.category}-${t.name}`;
                      const isEditing = editAssetTypeId === tId;
                      return (
                        <tr key={tId} style={{ borderBottom: "1px solid var(--n100)", background: isEditing ? "var(--p50)" : "transparent" }}>
                          <td style={{ padding: 8, textTransform: "capitalize" }}>{t.category}</td>
                          <td style={{ padding: 8 }}>{t.name}</td>
                          <td style={{ padding: 8, textAlign: "right" }}>
                            <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}>
                              <button className="btn btn-outline btn-xs btn-icon" title="Editar" onClick={() => startEditAssetType(t)}><Pencil size={12} /></button>
                              <button className="btn btn-outline btn-xs btn-icon" style={{ color: "#ef4444" }} title="Excluir" onClick={() => requestDeleteAssetType(t)}><Trash2 size={12} /></button>
                            </div>
                          </td>
                        </tr>
                      );
